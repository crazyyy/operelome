...
function wpeHeaderScripts() {
  if (!is_admin()) {
    ...
    wp_register_script('jquery', get_template_directory_uri() .  '/js/jquery.js', array(), '1.11.0', true); // Google CDN jQuery   RU: Регистрирую JQuery с хостинга Google
    wp_enqueue_script('jquery'); // Enqueue it!    RU: Подключаю его

    wp_register_script('modernizr', get_template_directory_uri() .  '/js/modernizr.js', array(), '2.8.3', true); // Modernizr
...
function wpeHeadNavTwo()
{
  wp_nav_menu(
  array(
    'theme_location'  => 'header-menu-second',
    'menu'            => '',
    'container'       => 'div',
    'container_class' => 'menu-{menu slug}-container',
    'container_id'    => '',
...
function wpeFootNavSec() {
  ...
  array(
    ...
    'link_before'     => '',
    'link_after'      => '',
    'items_wrap'      => '<ul class="nav-slidebar">%3$s</ul>',
    'depth'           => 0,
    'walker'          => ''
    )
  );
...
function register_cpt_faq() {
  $labels = array(
    ...
    'add_new' => _x( 'Добавить', 'faq' ),
    'add_new_item' => _x( 'Добавить faq', 'faq' ),
    'edit_item' => _x( 'Редактироваь faq', 'faq' ),
    'new_item' => _x( 'Новый faq', 'faq' ),
    'view_item' => _x( 'Посмотреть faq', 'faq' ),
    'search_items' => _x( 'Искать faq', 'faq' ),
